...
class EtsyOrdersSyncer extends EtsySyncer{
    ...
    public function get_etsy_order_by_receipt_id($receipt_id, $shop_id = 0) {
        $shop_id = $shop_id == 0 ? $this->get_shop_id() : $shop_id;
        return $this->api->getShopReceipt( [
            'params' => [
    ...
    private function sync_order_status($order, $status){
        $this->maybe_switch_off_emails();
        $status = apply_filters('platy_etsy_order_status', $status); 
    ...
    private function sync_order_by_receipt($order, $receipt_id, $shop_id){
        ...
        $this->sync_order_status($order, "wc-$new_status");
    ...
    static function get_etsy_order_receipt_id($post_id){
        $order = wc_get_order($post_id);
        if(empty($order)) {
            throw new EtsySyncerException("No such oder $post_id");
        }
        return $order->get_meta( "etsy_receipt_id", true );
    }
}
